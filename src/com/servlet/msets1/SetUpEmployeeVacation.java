...
public class SetUpEmployeeVacation extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try
		{
			...
			PreparedStatement ps =con.prepareStatement("insert into tblemployeevacation (userID, siteID, vacationDetails, StartDate, endDate, status) values (?,?,?,?,?,?)");
			ps.setString(1,request.getParameter("userid"));
			ps.setString(2,request.getParameter("siteid"));
			ps.setString(3,request.getParameter("vacationdetails"));
			...
			int i = ps.executeUpdate();
			 if(i>0)
			 {
				 RequestDispatcher rd = request.getRequestDispatcher("jsp/SetUpEmployeeVacation.jsp");
